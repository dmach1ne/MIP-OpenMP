...
public:
	...
	Tensor3D* forward(const Tensor3D* input) override {
		...
		for (int i = 0; i < nH; i++){
			for (int j = 0; j < nW; j++){
				for (int k = 0; k < nC; k++){
					output->set_elem(i, j, k, max(0.0, input->get_elem(i, j, k)));
				}
			}
		}
...
public:
	...
	void init(int init_type) {
		...
		if (init_type == MEAN_INIT) {
			...
			ifstream fin(filename_weight);
			ifstream fin_bias(filename_bias);
			if (!fin.is_open()){
				cout << "Error: cannot open file " << filename_weight << endl;
			}
			if (!fin_bias.is_open()){
				cout << "Error: cannot open file " << filename_bias << endl;
			}
			for (int l = 0; l < fC_out; l++){       // 출력 채널 먼저
				for (int k = 0; k < fC_in; k++){    // 입력 채널
					for (int i = 0; i < fK; i++){   // 행
						for (int j = 0; j < fK; j++){   // 열
							fin >> weight_tensor[i][j][k][l];
						}
					}
				}
			}
			for (int i = 0; i < fC_out; i++){
				fin_bias >> bias_tensor[i];
			}
			fin.close();
			fin_bias.close();
		}
	...
	Tensor3D* forward(const Tensor3D* input) override {
		...
		int outW = nW - fK + 1;
		int outC = fC_out;
		Tensor3D* output = new Tensor3D(nH, nW, outC);
		
		int offsetH = (nH - outH) / 2;
		int offsetW = (nW - outW) / 2;
					...
					for (int kh = 0; kh < fK; kh++) {
						for (int kw = 0; kw < fK; kw++) {
							for (int ic = 0; ic < fC_in; ic++) {
								sum += weight_tensor[kh][kw][ic][oc] * input->get_elem(oh + kh, ow + kw, ic);
							}
						}
					}
					sum += bias_tensor[oc];
					output->set_elem(offsetH + oh, offsetW + ow, oc, sum);
				}
			}
		}
		cout << name << " is finished" << endl;
